...
def pt_1(path_1, fenzhi):
    ...
    for ii in name_fwpmt:
        if ii[:-10] in ZDDM_list:
            ...
            print('找不到编号', ii[:-10])
            xlsx_jieguo_sheet.cell(row=numb, column=0).value = ii[:-10]
            xlsx_jieguo_sheet.cell(row=numb + 1, column=1).value = '有这个东西的' + fenzhi + '但是在表里面找不到对应的房屋预编号'
...
if __name__ == '__main__':
    ...
    path_xlsx = input().replace('\\', '/') or 'D:/test/新建文件夹/三桥不动产单元图/新建文件夹'
    try:
    ...
    ZDMJ_list = []
    JZMJ_list = []
    path_fwpmt = input('房屋平面图所在位置').replace('\\', '/') or 'D:/test/新建文件夹/三桥不动产单元图/bc11769f3b484ad1a5c4eff629cebb3b'
    path_zdt = input('宗地图所在位置').replace('\\', '/') or 'D:/test/新建文件夹/三桥不动产单元图/bc11769f3b484ad1a5c4eff629cebb3b'
